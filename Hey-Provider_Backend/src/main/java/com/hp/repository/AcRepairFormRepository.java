...
public interface AcRepairFormRepository extends CrudRepository<AcRepairFormEntity, Long>{
	...
	@Query("SELECT new com.hp.dto.AcRepairFormDatadto(a.id,a.name,a.number,a.date,a.time,a.addresss1,a.landmark,a.city,a.pincode,a.total,r.userName,p.userName,a.activeStatus,a.end_services_otp,a.window_ac_service,a.split_ac_service,a.less_no_cooling,a.ac_not_starting,a.noise_issue,a.water_leakege) FROM AcRepairFormEntity a JOIN Registration r on a.userid=r.id JOIN Provider p on a.providerid=p.id")
	public List<AcRepairFormDatadto> getAllUserServiceData();

	@Query("SELECT new com.hp.dto.AcRepairFormDatadto(a.id,a.name,a.number,a.date,a.time,a.addresss1,a.end_services_otp,a.landmark,a.city,a.pincode,a.total,r.userName,p.userName,a.activeStatus) FROM AcRepairFormEntity a JOIN Registration r on a.userid=r.id JOIN Provider p on a.providerid=p.id WHERE a.id = :id")
	public AcRepairFormDatadto findServiceById(@Param("id") long id);
	
	@Query("SELECT new com.hp.dto.AcRepairFormDatadto(a.id,a.name,a.number,a.date,a.time,a.addresss1,a.end_services_otp,a.landmark,a.city,a.pincode,a.total,r.userName,p.userName,a.activeStatus) FROM AcRepairFormEntity a JOIN Registration r on a.userid=r.id JOIN Provider p on a.providerid=p.id WHERE a.userid = :id")
	...
	@Query("SELECT new com.hp.dto.AcRepairFormDatadto(a.id,a.name,a.number,a.date,a.time,a.addresss1,a.landmark,a.city,a.pincode,a.total,r.userName,p.userName,a.activeStatus,a.end_services_otp,a.window_ac_service,a.split_ac_service,a.less_no_cooling,a.ac_not_starting,a.noise_issue,a.water_leakege) FROM AcRepairFormEntity a JOIN Registration r on a.userid=r.id JOIN Provider p on a.providerid=p.id WHERE a.activeStatus= 0")
